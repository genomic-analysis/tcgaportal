...
################# cut maximu followup 5000 days
                                        #m <- m[,which(as.numeric(m[5,])<4800)]
#################
...
if(ncol(mH)>0 && ncol(mL)>0){
    ...
    png("TCGA_survival_curve.png",width=6,height=5,res=100,units='in')
                                        #setEPS()
                                        #postscript("TCGA_survival_curve.eps",width=6,height=5)

    par(mar=c(5,6,2,0),mgp=c(3.5,1,0))
    plot(fit,col=c(1:2),xlim=c(0,350),frame=F,mark.time=T,lwd=3,yaxt="n",xaxt="n",xlab="Months",ylab="Survival Probability",cex.lab=2.5)
    legend(400,0.95,legend=c("high","low"),col=c(2:1),bty="n",lwd=4,cex=2)
    legend(100, 1.1, legend=paste(expression("logRank p="),format(p.val,digits=2)) ,bty="n",cex=2)
    axis(1,cex.axis=2,at=seq(0,300,by=100),labels=c("0","100","200","300"),cex.axis=2.5,lwd=3,mgp=c(0,1.5,0))
