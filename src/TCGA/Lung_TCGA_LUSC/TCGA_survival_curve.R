...
if(devideType=="user"){
    mL<-m[,which(as.numeric(m[10,])<=lowCut)]
    mH<-m[,which(as.numeric(m[10,])>highCut)]
    m<-cbind(mL,mH)
    MGAM2_type<-ifelse(as.numeric(m[10,])<=lowCut,0,1)
}else if(devideType=="median"){
    MGAM2_median<-median(as.numeric(m[10,]))
    MGAM2_type<-ifelse(as.numeric(m[10,])<=MGAM2_median,0,1)
    mL<-m[,which(as.numeric(m[10,])<=MGAM2_median)]
    mH<-m[,which(as.numeric(m[10,])>MGAM2_median)]
}else if(devideType=="tertile"){
    temp<-sort(as.numeric(m[10,]))
    ...
    m<-cbind(mL,mH)
    MGAM2_type<-ifelse(as.numeric(log2(as.numeric(m[10,])+0.01))<=(meanVal-sd),0,1)    
}

                                        #------------------------draw curve----------------------------------
if(ncol(mH)>0 && ncol(mL)>0){
    vital<-m[8,]
    followupMonth<-(as.numeric(m[9,])/30.0)
    vital[vital=="Alive"]<-0
    vital[vital=="Dead"]<-1
    ...
    dev.off()
                                        ...
                                        #-----------------------------------------draw expression
    c1 <- data.frame(group="High",value=as.numeric(mH[10,]))
    c2 <- data.frame(group="Low",value=as.numeric(mL[10,]))
    df <- rbind(c1,c2)
...
tmp <- dm[,5:ncol(dm)]
noNormalM <- tmp[c(7,1,10),which(tmp[3,]!="Solid Tissue Normal")]
...
m1 <- as.numeric(noNormalM[3,which(noNormalM[1,]=="basal")])
m2 <- as.numeric(noNormalM[3,which(noNormalM[1,]=="classical")])
m3 <- as.numeric(noNormalM[3,which(noNormalM[1,]=="primitive")])
m4 <- as.numeric(noNormalM[3,which(noNormalM[1,]=="secretory")])
...
d <- rbind(c("pval","basal","classical","primitive","secretory"),
c("basal",a1,a2,a3,a4),
c("classical",b1,b2,b3,b4),
c("primitive",c1,c2,c3,c4),
c("secretory",d1,d2,d3,d4))
write.table(d,"TCGA_primary_tumor_subtype_boxplot.wilcox",quote=FALSE,sep=",",row.names=FALSE,col.names=FALSE)


                                        #-------------------------draw tumor stage distribution boxplot and save wilcoxo-test pvalue----------------------------
tmp <- dm[,5:ncol(dm)]
noNormalM <- tmp[c(4,1,10),which(tmp[3,]!="Solid Tissue Normal")]
rownames(noNormalM) <- c("group","barcode","value")
colnames(noNormalM) <- NULL
...
m1 <- as.numeric(noNormalM[3,which(grepl("T1", noNormalM[1,]))])
m2 <- as.numeric(noNormalM[3,which(grepl("T2", noNormalM[1,]))])
m3 <- as.numeric(noNormalM[3,which(grepl("T3", noNormalM[1,]))])
m4 <- as.numeric(noNormalM[3,which(grepl("T4", noNormalM[1,]))])
a1<-round(as.numeric(wilcox.test(m1,m1,paired=FALSE)$p.value),digits=3)
a2<-round(as.numeric(wilcox.test(m1,m2,paired=FALSE)$p.value),digits=3)
a3<-round(as.numeric(wilcox.test(m1,m3,paired=FALSE)$p.value),digits=3)
a4<-round(as.numeric(wilcox.test(m1,m4,paired=FALSE)$p.value),digits=3)
