...
for(i in (6:rowMax-1)){
    ...
    ggplot(df,aes(x=group, y=value,fill=group))+stat_boxplot(geom="errorbar",width=0.4)+geom_boxplot(lwd=1.0,color="black",outlier.shape=NA)+geom_jitter(position=position_jitter(width=0.2),size=2,alpha=1)+labs(y=paste(geneName," log2(FPKM)"),x=gsub("_"," ",typeName))+theme(panel.grid.major = element_blank(), panel.background = element_blank(),panel.border=element_blank(),axis.line=element_line(colour="black",size=1),axis.ticks.x=element_line(size=1,colour="black"),axis.ticks.y=element_line(size=1,colour="black"),axis.text.x=element_text(size=25,face="plain",color="black"),axis.text.y=element_text(size=25,face="plain",color="black"),axis.title.x=element_text(face="plain",size=15,vjust=0.4),axis.title.y=element_text(face="plain",size=15,vjust=0.4),legend.position="none",line=element_line(size=2,colour="black"))+theme(axis.text.x=element_text(angle=angle,hjust=hjust))+theme(plot.margin=unit(c(0.4,0.1,0.1,0.4),"in"))
